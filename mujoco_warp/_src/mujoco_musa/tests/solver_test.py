...
from mujoco_warp._src.mujoco_musa import SolverType
...
class SolverTest(parameterized.TestCase):
  ...
  def test_init_linesearch(self):
    ...
    for keyframe in range(3):
      ...
      d.efc.jv.zero_()
      d.efc.quad.zero_()
      step_size_cost = ax.empty((d.nworld, m.opt.ls_iterations), dtype=float)
      solver_musa._linesearch(m, d, step_size_cost)
  ...
  def test_parallel_linesearch(self, cone):
    ...
    forward_musa.fwd_acceleration(m, d, factorize=True)
    solver_musa.solve(m, d)
    ...
    step_size_cost = ax.empty((d.nworld, 0), dtype=float)
    solver_musa._linesearch(m, d, step_size_cost)
    ...
    d.efc.Ma = ax.array2d(d_efc_Ma)
    d.efc.Jaref = ax.array(d_efc_Jaref)
    d.qacc = ax.array2d(d_qacc)
    step_size_cost = ax.empty((d.nworld, m.opt.ls_iterations), dtype=float)
    solver_musa._linesearch(m, d, step_size_cost)
  ...
  def test_solve_batch(self, cone, solver_, iterations, ls_iterations):
    """Tests solve (batch)."""
    mjm0, mjd0, _, _ = musa_test_data.fixture(
      "humanoid/humanoid.xml",
      keyframe=0,
    ...
    mjm1, mjd1, _, _ = musa_test_data.fixture(
      "humanoid/humanoid.xml",
      keyframe=2,
      overrides={"opt.cone": cone, "opt.solver": solver_, "opt.iterations": iterations, "opt.ls_iterations": ls_iterations},
    )
    ...
    mjm2, mjd2, _, _ = musa_test_data.fixture(
      "humanoid/humanoid.xml",
      keyframe=1,
      overrides={"opt.cone": cone, "opt.solver": solver_, "opt.iterations": iterations, "opt.ls_iterations": ls_iterations},
    )
    ...
    mjm, mjd, m, _ = musa_test_data.fixture(
      "humanoid/humanoid.xml",
      overrides={"opt.cone": cone, "opt.solver": solver_, "opt.iterations": iterations, "opt.ls_iterations": ls_iterations},
    )
    d_mjw = mj_musa.put_data(mjm, mjd, nworld=3, njmax=2 * nefc_active)
    d = d_mjw

    d.nefc = ax.array([nefc_active, nefc_active, nefc_active], dtype=ax.int32, ndim=1)
    d.ne = ax.array([ne_active, ne_active, ne_active], dtype=ax.int32, ndim=1)
    ...
    d.qacc_warmstart = ax.from_numpy(qacc_warmstart, dtype=ax.float32)
    d.qM = ax.from_numpy(qM, dtype=ax.float32)
    d.qacc_smooth = ax.from_numpy(qacc_smooth, dtype=ax.float32)
    d.qfrc_smooth = ax.from_numpy(qfrc_smooth, dtype=ax.float32)
    d.efc.J = ax.from_numpy(efc_J_fill, dtype=ax.float32)
    d.efc.D = ax.from_numpy(efc_D_fill, dtype=ax.float32)
    d.efc.aref = ax.from_numpy(efc_aref_fill, dtype=ax.float32)

    if solver_ == SolverType.CG:
      m0_mjw = mj_musa.put_model(mjm0)
      d0_mjw = mj_musa.put_data(mjm0, mjd0)
      smooth_musa.factor_m(m0_mjw, d0_mjw)
      qLD0 = d0_mjw.qLD.numpy()

      m1_mjw = mj_musa.put_model(mjm1)
      d1_mjw = mj_musa.put_data(mjm1, mjd1)
      smooth_musa.factor_m(m1_mjw, d1_mjw)
      qLD1 = d1_mjw.qLD.numpy()

      m2_mjw = mj_musa.put_model(mjm2)
      d2_mjw = mj_musa.put_data(mjm2, mjd2)
      smooth_musa.factor_m(m2_mjw, d2_mjw)
      qLD2 = d2_mjw.qLD.numpy()

      qLD = np.vstack([qLD0, qLD1, qLD2])
      d.qLD = ax.from_numpy(qLD, dtype=ax.float32)
  ...
  def test_frictionloss(self):
    ...
    for keyframe in range(3):
      _, mjd, m, d = musa_test_data.fixture("constraints.xml", keyframe=keyframe)
      solver_musa.solve(m, d)
