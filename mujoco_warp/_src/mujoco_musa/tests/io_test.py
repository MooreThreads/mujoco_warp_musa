...
from mujoco_warp._src.mujoco_musa import io as io_musa
from mujoco_warp._src.mujoco_musa.forward import step
...
class IOTest(parameterized.TestCase):
  def test_make_put_data(self):
    ...
    mjm, _, _, d = musa_test_data.fixture("pendula.xml")
    md = io_musa.make_data(mjm, nconmax=512, njmax=512)
  ...
  def test_get_data_into_m(self):
    ...
    m = io_musa.put_model(mjm)
    d = io_musa.put_data(mjm, mjd)

    mjd.qLD.fill(-123)
    mjd.qM.fill(-123)

    io_musa.get_data_into(mjd, mjm, d)
  ...
  def test_get_data_into(self, nworld, world_id):
    ...
    mjm, mjd, _, d = musa_test_data.fixture("humanoid/humanoid.xml", keyframe=0, nworld=nworld)
    ...
    io_musa.get_data_into(mjd, mjm, d, world_id=world_id)
  ...
  def test_static_geom_collision_with_put_data(self):
    ...
    m = io_musa.put_model(mjm)
    d = io_musa.put_data(mjm, mjd, nconmax=16, njmax=16)
  ...
  def test_reset_data(self, xml):
    ...
    ax.copy(d.nacon, ax.array([naconmax], dtype=int))
    io_musa.reset_data(m, d)
  ...
  def test_reset_data_world(self):
    ...
    m = io_musa.put_model(mjm)
    d = io_musa.make_data(mjm, nworld=2)

    # nonzero values
    qvel = ax.array(np.array([[1.0], [2.0]]), dtype=float)
    ...
    reset10 = ax.array(np.array([True, False]), dtype=bool)
    io_musa.reset_data(m, d, reset=reset10)
    ...
    reset00 = ax.array(np.array([False, False], dtype=bool))
    io_musa.reset_data(m, d, reset=reset00)
  ...
  def test_sdf(self):
    ...
    mjm, mjd, m, d = musa_test_data.fixture("collision_sdf/cow.xml")

    self.assertIsInstance(m.oct_aabb, ax.array)
    self.assertEqual(m.oct_aabb.dtype, ax.vec3)
  ...
  def test_ls_parallel(self):
    _, _, m, _ = musa_test_data.fixture(
      xml="""
    <mujoco>
    </mujoco>
    """
