...
from mujoco_warp._src.mujoco_musa import BiasType
from mujoco_warp._src.mujoco_musa import DisableBit
from mujoco_warp._src.mujoco_musa import EnableBit
from mujoco_warp._src.mujoco_musa import GainType
from mujoco_warp._src.mujoco_musa import IntegratorType
from mujoco_warp._src.mujoco_musa import forward as forward_musa
from mujoco_warp._src.mujoco_musa import solver as solver_musa
from mujoco_warp._src.mujoco_musa.tests import musa_test_data
from mujoco_warp.musa_api import put_data
from mujoco_warp.musa_api import put_model
...
class ForwardTest(parameterized.TestCase):
  ...
  def test_fwd_velocity(self, xml):
    _, mjd, m, d = musa_test_data.fixture(xml, qvel_noise=0.01, ctrl_noise=0.1)

    for arr in (d.actuator_velocity, d.qfrc_bias):
      arr.zero_()

    forward_musa.fwd_velocity(m, d)
  ...
  def test_fwd_velocity_tendon(self):
    _, mjd, m, d = musa_test_data.fixture("tendon/fixed.xml")
  ...
  def test_actuation(self, xml, disableflags):
    mjm, mjd, m, d = musa_test_data.fixture(xml, keyframe=0, overrides={"opt.disableflags": disableflags})

    for arr in (d.qfrc_actuator, d.actuator_force, d.act_dot):
      arr.zero_()

    forward_musa.fwd_actuation(m, d)
  ...
  def test_clampctrl(self, disableflags):
    _, mjd, _, d = musa_test_data.fixture(
      xml="""
    <mujoco>
      <worldbody>
        <body>
  ...
  def test_fwd_acceleration(self):
    _, mjd, m, d = musa_test_data.fixture("humanoid/humanoid.xml", qvel_noise=0.01, ctrl_noise=0.1)

    for arr in (d.qfrc_smooth, d.qacc_smooth):
      arr.zero_()

    forward_musa.fwd_acceleration(m, d)
  ...
  def test_euler(self, jacobian, disableflags):
    mjm, mjd, _, _ = musa_test_data.fixture(
      "pendula.xml",
      qvel_noise=0.01,
      ctrl_noise=0.1,
    ...
    # TODO: remove m_, d_
    solver_musa.solve(m, d)  # compute efc.Ma
    forward_musa.euler(m, d)
  ...
  def test_rungekutta4(self):
    ...
    def rk_step() -> ax.array(dtype=ax.float32, ndim=2):
      d.qpos = ax.empty_like(d.qpos)
      d.qpos.fill_(1)
      d.qvel = ax.empty_like(d.qvel)
      d.qvel.fill_(1)
      d.act = ax.empty_like(d.act)
      d.act.fill_(1)
      forward_musa.rungekutta4(m, d)
      return d.qpos
  ...
  @absltest.skipUnless(ax.config.enable_mempools_at_init, "not implemented: axinfra graph")
  @parameterized.product(
  ...
  def test_graph_capture(self, xml, graph_conditional):
    ...
    _, _, m, d = musa_test_data.fixture(xml, overrides={"opt.graph_conditional": graph_conditional})
  ...
  def test_tendon_actuator_force_limits(self):
    for keyframe in range(7):
      _, mjd, m, d = musa_test_data.fixture("actuation/tendon_force_limit.xml", keyframe=keyframe)

      d.actuator_force.zero_()

      forward_musa.forward(m, d)
  ...
  def test_step1(self, xml, energy):
    ...
    for arr in step1_field:
      ...
      if arr in ["xmat", "ximat", "geom_xmat", "site_xmat", "cam_xmat"]:
        ...
        vec_wp = ax.array(vec, dtype=float)
        res_wp = ax.zeros((1, mjm.nv), dtype=float)
        mj_musa.solve_m(m, d, res_wp, vec_wp)
      ...
      if "efc_" in arr:
        if d_arr.ndim == 1:
          # 1D
          d_sort_indices = np.argsort(d_arr)
          d_sorted_arr = d_arr[d_sort_indices]
          mjd_sort_indices = np.argsort(mjd_arr)
          mjd_sorted_arr = mjd_arr[mjd_sort_indices]
          _assert_eq(d_sorted_arr, mjd_sorted_arr, arr)
        else:
          # 2D efc_J
          d_efc_feature = np.sum(d_arr, axis=1)
          mjd_efc_feature = np.sum(mjd_arr, axis=1)
          d_sort_indices = np.argsort(d_efc_feature)
          d_sorted_arr = d_arr[d_sort_indices].reshape(-1)
          mjd_sort_indices = np.argsort(mjd_efc_feature)
          mjd_sorted_arr = mjd_arr[mjd_sort_indices].reshape(-1)
          _assert_eq(d_sorted_arr, mjd_sorted_arr, arr)
      else:
  ...
  def test_step2(self, xml, integrator):
    mjm, mjd, m, _ = musa_test_data.fixture(xml, qvel_noise=0.01, ctrl_noise=0.1, overrides={"opt.integrator": integrator})
  ...
  def test_step_no_dofs(self):
    ...
    _, _, m, d = musa_test_data.fixture(
      xml="""
    <mujoco>
      <body>
        <geom type="sphere" size="1"/>
