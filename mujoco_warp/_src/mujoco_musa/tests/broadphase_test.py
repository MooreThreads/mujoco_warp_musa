...
from mujoco_warp._src.mujoco_musa import collision_driver
from mujoco_warp._src.mujoco_musa import types as mjmtp
from mujoco_warp._src.mujoco_musa.tests import musa_test_data
from mujoco_warp.musa_api import BroadphaseFilter
from mujoco_warp.musa_api import BroadphaseType
from mujoco_warp.musa_api import DisableBit


def broadphase_caller(m: mjmtp.Model, d: mjmtp.Data):
...
class BroadphaseTest(parameterized.TestCase):
  ...
  def test_broadphase(self, broadphase, filter):
    ...
    mjm, _, m, d0 = musa_test_data.fixture(xml=_XML, keyframe=0)
    ...
    _, mjd1, _, d1 = musa_test_data.fixture(xml=_XML, keyframe=1)
    ...
    _, mjd2, _, d2 = musa_test_data.fixture(xml=_XML, keyframe=2)
    ...
    d3 = mj_musa.make_data(mjm, nworld=2, nconmax=512, njmax=512)
    d3.geom_xpos = ax.array(
      np.vstack([np.expand_dims(mjd1.geom_xpos, axis=0), np.expand_dims(mjd2.geom_xpos, axis=0)]),
      dtype=ax.vec3,
    )
    d3.geom_xmat = ax.array(
  ...
  def test_broadphase_margin(self, margin1, margin2, ncollision):
    ...
    """
    _, _, m, d = musa_test_data.fixture(xml=_MJCF, keyframe=0)
  ...
  def test_broadphase_filterparent(self, disablebit, expected_collisions):
    ...
    """
    _, _, m, d = musa_test_data.fixture(xml=_MJCF, keyframe=0, overrides={"opt.disableflags": disablebit})
  ...
  def test_broadphase_filter(self):
    ...
    _, _, m, d = musa_test_data.fixture(xml=_PLANE_CAPSULE_CAPSULE, keyframe=0)
    ...
    _, _, m, d = musa_test_data.fixture(xml=_PLANE_CAPSULE_CAPSULE, keyframe=1)
    m.opt.broadphase_filter = plane_sphere
    broadphase_caller(m, d)
    ...
    _, _, m, d = musa_test_data.fixture(xml=_PLANE_CAPSULE_CAPSULE, keyframe=2)
    m.opt.broadphase_filter = plane_sphere
    broadphase_caller(m, d)
    self.assertEqual(d.ncollision.numpy()[0], 1)
    ...
    _, _, m, d = musa_test_data.fixture(xml=_PLANE_CAPSULE_CAPSULE, keyframe=3)
    m.opt.broadphase_filter = plane_sphere
    broadphase_caller(m, d)
    self.assertEqual(d.ncollision.numpy()[0], 1)
