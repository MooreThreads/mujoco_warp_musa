...
def musa_module(musa_func):
  def decorator(func):
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
      if config.use_musa:
        if config.print_log:
          module_name = musa_func.__name__
          if module_name not in musa_module_log:
            musa_module_log.add(module_name)
            print(f"[Info] run MUSA module: {module_name}")
        return musa_func(*args, **kwargs)
      else:
        return func(*args, **kwargs)
    return wrapper
  return decorator

def copy_wp_array_batch(arrlist: list[tuple]):
  for dst, src in arrlist:
    ax.copy_wp_array(dst, src)

def get_nested_attr(obj, attr_path):
    attrs = attr_path.split('.')
    # nested loop to access nested attributes
    for attr in attrs:
        obj = getattr(obj, attr)
    return obj
def copy_wp_array_batch_attrlist(dst_obj, src_obj, attrlist: list[str]):
  for name in attrlist:
    # allow for nested attributes, like "efc.type"
    if '.' in name:
      # nested attirbutes
      dst = get_nested_attr(dst_obj, name)
      src = get_nested_attr(src_obj, name)
    else:
      # simple attributes
      dst = getattr(dst_obj, name)
      src =  getattr(src_obj, name)
    ax.copy_wp_array(dst, src)

def check_array_equal(a, b, name: str = "", tolerance: float = 1e-3):
  a_np = a.numpy()
  b_np = b.numpy()

  # np.testing.assert_allclose(a_np, b_np, atol=tolerance, rtol=tolerance)

  err = a_np - b_np
  if np.linalg.norm(err.flatten(), ord=np.inf) > tolerance:
    err_msg = f"Array not equal: {name} \nA: {a_np}\nB: {b_np}"
    print(err_msg)
  else:
    print(f"Array equal: {name}")

def check_array_equal_batch_attrlit(a_obj, b_obj, attrlist: list[str], tolerance: float = 1e-3):
  for name in attrlist:
    a = getattr(a_obj, name)
    b = getattr(b_obj, name)
    check_array_equal(a, b, name, tolerance)
