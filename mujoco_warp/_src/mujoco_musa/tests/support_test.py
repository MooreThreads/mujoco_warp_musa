...
from mujoco_warp._src.mujoco_musa import State
from mujoco_warp._src.mujoco_musa import support as support_musa
...
class SupportTest(parameterized.TestCase):  # noqa: D101
  ...
  def test_mul_m(self, jacobian):
    ...
    res = ax.zeros((1, mjm.nv), dtype=ax.float32)
    vec = ax.from_numpy(np.expand_dims(mj_vec, axis=0), dtype=ax.float32)
    support_musa.mul_m(m, d, res, vec)
  ...
  def test_xfrc_accumulated(self):
    ...
    d.xfrc_applied = ax.from_numpy(xfrc, dtype=ax.spatial_vector)
    qfrc = ax.zeros((1, mjm.nv), dtype=ax.float32)
    support_musa.xfrc_accumulate(m, d, qfrc)
  ...
  # @absltest.skipUnless(hasattr(support_musa, "contact_force"), "not implemented: support.contact_force")
  ...
  #     </mujoco>
  #   """
  #   mjm, mjd, m, d = musa_test_data.fixture(xml=_CONTACT, keyframe=0, overrides={"opt.cone": cone})

  #   mj_force = np.zeros(6, dtype=float)
  #   mujoco.mj_contactForce(mjm, mjd, 0, mj_force)

  #   contact_ids = ax.zeros(1, dtype=int)
  #   force = ax.zeros(1, dtype=ax.spatial_vector)

  #   support_musa.contact_force(m, d, contact_ids, to_world_frame, force)
  ...
  # @absltest.skipUnless(hasattr(support_musa, "get_state"), "not implemented: support.get_state")
  ...
  #   mjw_state = ax.zeros((d.nworld, size), dtype=float)
  #   support_musa.get_state(m, d, mjw_state, State.INTEGRATION)

  #   _assert_eq(mjw_state.numpy()[0], mj_state, "state")

  #   d2 = mjwarp.put_data(mjm, mjd, nworld=2)
  #   mjw_state2 = ax.zeros((d2.nworld, size), dtype=float)
  #   active = ax.array([False, True], dtype=bool)
  #   support_musa.get_state(m, d2, mjw_state2, State.INTEGRATION, active=active)

  #   _assert_eq(mjw_state2.numpy()[0], 0, "state0")
  #   _assert_eq(mjw_state2.numpy()[1], mj_state, "state1")

  # @absltest.skipUnless(hasattr(support_musa, "set_state"), "not implemented: support.set_state")
  ...
  #   mjw_state = ax.array(np.expand_dims(mj_state, axis=0), dtype=float)
  #   support_musa.set_state(m, d, mjw_state, State.INTEGRATION)
  ...
  #   mjw_state2 = ax.array(np.tile(mj_state, (2, 1)), dtype=float)
  #   active = ax.array([False, True], dtype=bool)
  #   support_musa.set_state(m, d2, mjw_state2, State.INTEGRATION, active=active)
