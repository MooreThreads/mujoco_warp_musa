...
from mujoco_warp._src.mujoco_musa.io import find_keys
from mujoco_warp._src.mujoco_musa.io import make_trajectory
from mujoco_warp._src.mujoco_musa.io import override_model

# ax.launch = patched_launch
use_capture = False
# use_capture = True
custom_nworld = 1
headless = False
...
def _compile_step(m, d):
  ...
  with ax.ScopedCapture() as capture:
    mjw.step(m, d)
...
def _main(argv: Sequence[str]) -> None:
  ...
  if _ENGINE.value == EngineOptions.C:
    ...
    ax.config.quiet = flags.FLAGS["verbosity"].value < 1
    if _CLEAR_KERNEL_CACHE.value:
      ax.clear_kernel_cache()

    with ax.ScopedDevice(_DEVICE.value):
      m = mjw.put_model(mjm)
      ...
      d = mjw.put_data(mjm, mjd, nconmax=_NCONMAX.value, njmax=_NJMAX.value, nworld=custom_nworld)
  ...
  def run_sim(m, d, viewer = None):
   with ax.ScopedDevice(_DEVICE.value):
    opt = copy.copy(mjm.opt)

    nstep = 0
    while True:
      if nstep >= num_steps and num_steps > 0:
        break
      start = time.time()
      ...
      if _ENGINE.value == EngineOptions.C:
        ...
        ax.copy(d.ctrl, ax.array([mjd.ctrl.astype(np.float32)]))
        ax.copy(d.act, ax.array([mjd.act.astype(np.float32)]))
        ax.copy(d.xfrc_applied, ax.array([mjd.xfrc_applied.astype(np.float32)]))
        ax.copy(d.qpos, ax.array([mjd.qpos.astype(np.float32)]))
        ax.copy(d.qvel, ax.array([mjd.qvel.astype(np.float32)]))
        ax.copy(d.time, ax.array([mjd.time], dtype=ax.float32))
      ...
      if viewer is not None:
        viewer.sync()
      nstep += 1
      print(f"step {nstep}", end="\r", flush=True)

      if use_dump:
        if nstep == dump_step_id:
          from mujoco_warp.fsa_dump_warp import patched_launch, patched_launch_tile
          ax.launch = patched_launch
          ax.launch_tiled = patched_launch_tile
        if nstep > dump_step_id:
          break
...
def main():
  ...
  sys.argv[0] = "mujoco_warp.viewer_musa"
