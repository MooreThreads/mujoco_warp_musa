...
def _create_array(data: Any, spec: ax.array, sizes: dict[str, int]) -> Union[ax.array, None]:
  ...
  if data is None and shape is None:
    ...
    array = ax.zeros(shape, dtype=spec.dtype)
  else:
    array = ax.array(np.array(data), dtype=spec.dtype, shape=shape)
...
def put_model(mjm: mujoco.MjModel) -> types.Model:
  ...
  m.body_tree = tuple(ax.array(bodies[i], dtype=int) for i in sorted(bodies))
  ...
  m.qM_tiles = tuple(types.TileSet(adr=ax.array(tiles[sz], dtype=int), size=sz) for sz in sorted(tiles.keys()))
  ...
  for k in range(mjm.nv):
    ...
    while i > -1:
      ...
      Madr_ki -= 1
  m.qLD_updates = tuple(ax.array(qLD_updates[i], dtype=ax.vec3i) for i in sorted(qLD_updates))
...
def make_data(
  ...
  if is_sparse(mjm):
    d.qM = ax.zeros((nworld, 1, mjm.nM), dtype=float)
    d.qLD = ax.zeros((nworld, 1, mjm.nC), dtype=float)
  else:
    d.qM = ax.zeros((nworld, sizes["nv_pad"], sizes["nv_pad"]), dtype=float)
    d.qLD = ax.zeros((nworld, mjm.nv, mjm.nv), dtype=float)
  ...
  d.geom_xpos = ax.array(np.tile(mjd.geom_xpos, (nworld, 1)), shape=(nworld, mjm.ngeom), dtype=ax.vec3)
  d.geom_xmat = ax.array(np.tile(mjd.geom_xmat, (nworld, 1)), shape=(nworld, mjm.ngeom), dtype=ax.mat33)
...
def put_data(
  ...
  contact.efc_address = ax.array(contact.efc_address, dtype=int)
  contact.worldid = np.pad(np.repeat(np.arange(nworld), mjd.ncon), (0, naconmax - nworld * mjd.ncon))
  contact.worldid = ax.array(contact.worldid, dtype=int)
  contact.type = ax.ones((naconmax,), dtype=int)  # TODO(team): set values
  contact.geomcollisionid = ax.zeros((naconmax,), dtype=int)  # TODO(team): set values
  ...
  for f in dataclasses.fields(types.Constraint):
    ...
    efc_kwargs[f.name] = ax.array(val, dtype=f.type.dtype)
  ...
  efc.J = ax.array(efc.J, dtype=float)
  ...
  d = types.Data(**d_kwargs)
  d.solver_niter = ax.full((nworld,), mjd.solver_niter[0], dtype=int)

  if is_sparse(mjm):
    d.qM = ax.array(np.full((nworld, 1, mjm.nM), mjd.qM), dtype=float)
    d.qLD = ax.array(np.full((nworld, 1, mjm.nC), mjd.qLD), dtype=float)
  else:
    ...
    d.qM = ax.array(np.full((nworld, sizes["nv_pad"], sizes["nv_pad"]), qM_padded), dtype=float)
    d.qLD = ax.array(np.full((nworld, mjm.nv, mjm.nv), qLD), dtype=float)

  if mujoco.mj_isSparse(mjm):
    ten_J = np.zeros((mjm.ntendon, mjm.nv))
    mujoco.mju_sparse2dense(ten_J, mjd.ten_J.reshape(-1), mjd.ten_J_rownnz, mjd.ten_J_rowadr, mjd.ten_J_colind.reshape(-1))
    d.ten_J = ax.array(np.full((nworld, mjm.ntendon, mjm.nv), ten_J), dtype=float)
    ...
    d.flexedge_J = ax.array(np.full((nworld, mjm.nflexedge, mjm.nv), flexedge_J), dtype=float)
  else:
  ...
  d.actuator_moment = ax.array(np.full((nworld, mjm.nu, mjm.nv), actuator_moment), dtype=float)

  d.nacon = ax.array([mjd.ncon * nworld], dtype=int)
  d.ne_connect = ax.full(nworld, int(3 * np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_CONNECT) & mjd.eq_active)), dtype=int)
  d.ne_weld = ax.full(nworld, int(6 * np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_WELD) & mjd.eq_active)), dtype=int)
  d.ne_jnt = ax.full(nworld, int(np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_JOINT) & mjd.eq_active)), dtype=int)
  d.ne_ten = ax.full(nworld, int(np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_TENDON) & mjd.eq_active)), dtype=int)
  d.ne_flex = ax.full(nworld, int(np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_FLEX) & mjd.eq_active)), dtype=int)
  d.nsolving = ax.array([nworld], dtype=int)
...
def reset_data(m: types.Model, d: types.Data, reset: Optional[ax.array] = None):
  ...
  """
  reset_input = reset or ax.ones(d.nworld, dtype=bool)

  ax.launch("reset_data__reset_xfrc_applied", dim=(d.nworld, m.nbody, 6), inputs=[reset_input], outputs=[d.xfrc_applied])
  ax.launch(
    "reset_data__reset_qM",
