...
from mujoco_warp._src.mujoco_musa import ConeType
from mujoco_warp._src.mujoco_musa import DisableBit
from mujoco_warp._src.mujoco_musa import smooth as smooth_musa
from mujoco_warp._src.mujoco_musa.tests import musa_test_data
...
class SmoothTest(parameterized.TestCase):  # noqa: D101
  def test_mocap_kinematics(self):
    ...
    mjm, mjd, m, d = musa_test_data.fixture(xml=xml)
    ...
    d.mocap_pos = ax.array(np.array(new_mocap_pos, dtype=np.float32), dtype=ax.float32, shape=(1, 1, 3))
  ...
  def test_kinematics(self):
    """Tests kinematics."""
    _, mjd, m, d = musa_test_data.fixture("pendula.xml")
  ...
  def test_crb(self, jacobian):
    """Tests crb."""
    mjm, mjd, m, d = musa_test_data.fixture("pendula.xml", overrides={"opt.jacobian": jacobian})
  ...
  def test_factor_m(self, jacobian):
    """Tests factor_m."""
    _, mjd, m, d = musa_test_data.fixture("pendula.xml", overrides={"opt.jacobian": jacobian})

    qLD = d.qLD.numpy()[0].copy()
    for arr in (d.qLD, d.qLDiagInv):
      arr.zero_()

    smooth_musa.factor_m(m, d)
  ...
  def test_solve_m(self, jacobian):
    ...
    smooth_musa.solve_m(m, d, d.qacc_smooth, d.qfrc_smooth)
  ...
  def test_rne(self, gravity):
    """Tests rne."""
    _, mjd, m, d = musa_test_data.fixture("pendula.xml", overrides={"opt.disableflags": DisableBit.CONTACT | gravity})
  ...
  def test_rne_postconstraint(self, gravity):
    ...
    mjm, mjd, m, d = musa_test_data.fixture("pendula.xml", overrides={"opt.disableflags": DisableBit.CONTACT | gravity})

    mjd.xfrc_applied = np.random.uniform(low=-0.01, high=0.01, size=mjd.xfrc_applied.shape)
    d.xfrc_applied = ax.array(np.expand_dims(mjd.xfrc_applied, axis=0), dtype=ax.spatial_vector)
    ...
    smooth_musa.rne_postconstraint(m, d)
    ...
    _EQUALITY = """
      ...
      """
    mjm, mjd, m, d = musa_test_data.fixture(xml=_EQUALITY, qvel_noise=0.01, ctrl_noise=0.1, keyframe=0)
    ...
    mjm, mjd, m, d = musa_test_data.fixture("constraints.xml", keyframe=1, overrides={"opt.disableflags": DisableBit.EQUALITY})
  ...
  def test_transmission(self, xml):
    """Tests transmission."""
    mjm, mjd, m, d = musa_test_data.fixture(xml)
  ...
  def test_actuator_adhesion(self, keyframe, cone):
    ...
    mjm, mjd, m, d = musa_test_data.fixture("actuation/adhesion.xml", keyframe=keyframe, overrides={"opt.cone": cone})

    d.actuator_length.zero_()
    d.actuator_moment.zero_()
    mj_musa.collision(m, d)  # compute contact.includemargin
    mj_musa.make_constraint(m, d)  # compute contact.efc_address
    smooth_musa.transmission(m, d)
  ...
  def test_subtree_vel(self):
    """Tests subtree_vel."""
    mjm, mjd, m, d = musa_test_data.fixture("pendula.xml")

    for arr in (d.subtree_linvel, d.subtree_angmom):
      arr.zero_()

    mujoco.mj_subtreeVel(mjm, mjd)
    smooth_musa.subtree_vel(m, d)
  ...
  def test_tendon(self, xml):
    """Tests tendon."""
    mjm, mjd, m, d = musa_test_data.fixture(xml, keyframe=0)

    for arr in (d.ten_length, d.ten_J, d.actuator_length, d.actuator_moment):
      arr.zero_()

    smooth_musa.tendon(m, d)
    smooth_musa.transmission(m, d)
  ...
  def test_factor_solve_i(self, jacobian):
    mjm, mjd, m, d = musa_test_data.fixture(
      xml="""
    <mujoco>
      <worldbody>
        <body>
    ...
    d.qLD.fill_(ax.inf)
    if sparse:
      d.qLDiagInv.fill_(ax.inf)

    res = ax.zeros((1, mjm.nv), dtype=float)
    # vec = ax.ones((1, mjm.nv), dtype=float)
    vec = ax.empty((1, mjm.nv), dtype=float)
    vec.fill_(1.0)

    smooth_musa.factor_solve_i(m, d, d.qM, d.qLD, d.qLDiagInv, res, vec)
  ...
  def test_tendon_armature(self):
    mjm, mjd, m, d = musa_test_data.fixture("tendon/armature.xml", keyframe=0)

    # qM
    d.qM.zero_()

    smooth_musa.crb(m, d)
    smooth_musa.tendon_armature(m, d)
    ...
    smooth_musa.rne(m, d)
    smooth_musa.tendon_bias(m, d, d.qfrc_bias)
    _assert_eq(d.qfrc_bias.numpy()[0], mjd.qfrc_bias, "qfrc_bias")

  def test_flex(self):
    mjm, mjd, m, d = musa_test_data.fixture("flex/floppy.xml")
    d.flexvert_xpos.fill_(ax.inf)
    d.flexedge_length.fill_(ax.inf)
    d.flexedge_velocity.fill_(ax.inf)
    d.flexedge_J.fill_(ax.inf)

    smooth_musa.kinematics(m, d)
    smooth_musa.com_pos(m, d)
    smooth_musa.flex(m, d)
    mujoco.mj_kinematics(mjm, mjd)
