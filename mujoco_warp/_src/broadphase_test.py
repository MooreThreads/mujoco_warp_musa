...
from mujoco_warp._src.mujoco_musa import collision_driver as cd
from mujoco_warp._src.warp_util import musa_module


@musa_module(cd.broadphase_copydata)
...
class BroadphaseTest(parameterized.TestCase):
  ...
  def test_broadphase(self, broadphase, filter):
    ...
    m.musa_model.opt.broadphase = broadphase
    m.musa_model.opt.broadphase_filter = filter
  ...
  def test_broadphase_filter(self):
    ...
    m.musa_model.opt.broadphase_filter = plane_sphere
    broadphase_caller(m, d)
    self.assertEqual(d.ncollision.numpy()[0], 0)

    _, _, m, d = test_data.fixture(xml=_PLANE_CAPSULE_CAPSULE, keyframe=0)
    m.opt.broadphase_filter = plane_aabb
    m.musa_model.opt.broadphase_filter = plane_aabb
    broadphase_caller(m, d)
    self.assertEqual(d.ncollision.numpy()[0], 0)

    _, _, m, d = test_data.fixture(xml=_PLANE_CAPSULE_CAPSULE, keyframe=0)
    m.opt.broadphase_filter = plane_obb
    m.musa_model.opt.broadphase_filter = plane_obb
    broadphase_caller(m, d)
    self.assertEqual(d.ncollision.numpy()[0], 0)
    ...
    m.musa_model.opt.broadphase_filter = plane
    broadphase_caller(m, d)
