...
from mujoco_warp._src.mujoco_musa import constraint as constraint_musa
...
def _assert_efc_eq(d: mjmtp, mjd, nefc: int, name: str, nv: int):
...
class ConstraintTest(parameterized.TestCase):  # noqa: D101
  ...
  def test_condim(self, cone, condim1, condim2):
    ...
    _, mjd, m, d = musa_test_data.fixture(xml=xml, keyframe=0, overrides={"opt.cone": cone})
    ...
    constraint_musa.make_constraint(m, d)

    # sort efc.J by row sum
    nefc = mjd.nefc
    d_J_data = d.efc.J.numpy()[0, :nefc, :m.nv]
    d_J_features  = np.sum(d_J_data, axis=1)
    d_J_sort_indices = np.argsort(d_J_features)
    d_sorted_J = d_J_data[d_J_sort_indices].reshape(-1)
    # sort MJd.efc_J by row sum
    mjd_J_2d = mjd.efc_J.reshape(nefc, m.nv)
    mjd_J_features = np.sum(mjd_J_2d, axis=1)
    mjd_J_sort_indices = np.argsort(mjd_J_features)
    mjd_sorted_J = mjd_J_2d[mjd_J_sort_indices].reshape(-1)
    _assert_eq(d_sorted_J, mjd_sorted_J, "efc_J")
    ...
    # Compare sorted data
    d_sorted = d.efc.D.numpy()[0, d_sort_indices]
    _assert_eq(d_sorted, mjd_sorted_D, "efc_D")

    d_sorted = d.efc.aref.numpy()[0, d_sort_indices]
    _assert_eq(d_sorted, mjd_sorted_aref, "efc_aref")

    d_sorted = d.efc.pos.numpy()[0, d_sort_indices]
    _assert_eq(d_sorted, mjd_sorted_pos, "efc_pos")

    d_sorted = d.efc.margin.numpy()[0, d_sort_indices]
    _assert_eq(d_sorted, mjd_sorted_margin, "efc_margin")
  ...
  def test_constraints(self, cone):
    """Test constraints."""
    for key in range(3):
      _, mjd, m, d = musa_test_data.fixture("constraints.xml", keyframe=key, overrides={"opt.cone": cone})
  ...
  def test_limit_tendon(self):
    ...
    for keyframe in range(-1, 1):
      _, mjd, m, d = musa_test_data.fixture("tendon/tendon_limit.xml", keyframe=keyframe)
  ...
  def test_equality_tendon(self):
    ...
    _, mjd, m, d = musa_test_data.fixture(
      xml="""
      <mujoco>
        <option>
