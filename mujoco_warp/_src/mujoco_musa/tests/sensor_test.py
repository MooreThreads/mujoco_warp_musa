...
from mujoco_warp._src.mujoco_musa import collision_driver as collision_driver_musa
from mujoco_warp._src.mujoco_musa import forward as forward_musa
from mujoco_warp._src.mujoco_musa import sensor as sensor_musa
...
class SensorTest(parameterized.TestCase):
  def test_sensor(self):
    ...
    sensor_musa.sensor_pos(m, d)
    sensor_musa.sensor_vel(m, d)
    sensor_musa.sensor_acc(m, d)
  ...
  def test_tendon_sensor(self):
    """Test tendon sensors."""
    _, mjd, m, d = musa_test_data.fixture("tendon/fixed.xml", keyframe=0)
  ...
  #   _, mjd, m, d = musa_test_data.fixture(xml=_MJCF, keyframe=0)
  ...
  def test_sensor_collision(self, type0, type1, type2, type3):
    ...
    _, mjd, m, d = musa_test_data.fixture(xml=_MJCF)

    d.sensordata.fill_(ax.inf)
    smooth_musa.kinematics(m, d)
    collision_driver_musa.collision(m, d)
    sensor_musa.sensor_pos(m, d)
