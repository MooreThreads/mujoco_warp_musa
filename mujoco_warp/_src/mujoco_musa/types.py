...
def array(*args) -> ax.array:
  """A wrapper around axinfra.array that adds extra metadata to ease type introspection.
  ...
  arr = ax.array(ndim=len(shape), dtype=dtype)
  arr.shape = shape
...
class Option:
  ...
  gravity: array("*", ax.vec3)
  wind: array("*", ax.vec3)
  magnetic: array("*", ax.vec3)
  integrator: int
  cone: int
  solver: int
  iterations: int
...
class Model:
  ...
  body_pos: array("*", "nbody", ax.vec3)
  body_quat: array("*", "nbody", ax.quat)
  body_ipos: array("*", "nbody", ax.vec3)
  body_iquat: array("*", "nbody", ax.quat)
  body_mass: array("*", "nbody", float)
  body_subtreemass: array("*", "nbody", float)
  body_inertia: array("*", "nbody", ax.vec3)
  body_invweight0: array("*", "nbody", ax.vec2)
  body_gravcomp: array("*", "nbody", float)
  body_contype: array("nbody", int)
  body_conaffinity: array("nbody", int)
  oct_child: array("noct", ax.vec8i)
  oct_aabb: array("noct", 2, ax.vec3)
  oct_coeff: array("noct", ax.vec8)
  ...
  jnt_solref: array("*", "njnt", ax.vec2)
  jnt_solimp: array("*", "njnt", ax.vec5)
  jnt_pos: array("*", "njnt", ax.vec3)
  jnt_axis: array("*", "njnt", ax.vec3)
  jnt_stiffness: array("*", "njnt", float)
  jnt_range: array("*", "njnt", ax.vec2)
  jnt_actfrcrange: array("*", "njnt", ax.vec2)
  jnt_margin: array("*", "njnt", float)
  dof_bodyid: array("nv", int)
  dof_jntid: array("nv", int)
  dof_parentid: array("nv", int)
  dof_Madr: array("nv", int)
  dof_solref: array("*", "nv", ax.vec2)
  dof_solimp: array("*", "nv", ax.vec5)
  ...
  geom_solref: array("*", "ngeom", ax.vec2)
  geom_solimp: array("*", "ngeom", ax.vec5)
  geom_size: array("*", "ngeom", ax.vec3)
  geom_aabb: array("*", "ngeom", 2, ax.vec3)
  geom_rbound: array("*", "ngeom", float)
  geom_pos: array("*", "ngeom", ax.vec3)
  geom_quat: array("*", "ngeom", ax.quat)
  geom_friction: array("*", "ngeom", ax.vec3)
  geom_margin: array("*", "ngeom", float)
  geom_gap: array("*", "ngeom", float)
  geom_fluid: array("ngeom", 12, float)
  geom_rgba: array("*", "ngeom", ax.vec4)
  site_type: array("nsite", int)
  site_bodyid: array("nsite", int)
  site_size: array("nsite", ax.vec3)
  site_pos: array("*", "nsite", ax.vec3)
  site_quat: array("*", "nsite", ax.quat)
  cam_mode: array("ncam", int)
  cam_bodyid: array("ncam", int)
  cam_targetbodyid: array("ncam", int)
  cam_pos: array("*", "ncam", ax.vec3)
  cam_quat: array("*", "ncam", ax.quat)
  cam_poscom0: array("*", "ncam", ax.vec3)
  cam_pos0: array("*", "ncam", ax.vec3)
  cam_mat0: array("*", "ncam", ax.mat33)
  cam_fovy: array("ncam", float)
  cam_resolution: array("ncam", ax.vec2i)
  cam_sensorsize: array("ncam", ax.vec2)
  cam_intrinsic: array("ncam", ax.vec4)
  ...
  light_pos: array("*", "nlight", ax.vec3)
  light_dir: array("*", "nlight", ax.vec3)
  light_poscom0: array("*", "nlight", ax.vec3)
  light_pos0: array("*", "nlight", ax.vec3)
  light_dir0: array("*", "nlight", ax.vec3)
  ...
  flex_edge: array("nflexedge", ax.vec2i)
  flex_edgeflap: array("nflexedge", ax.vec2i)
  ...
  mesh_vert: array("nmeshvert", ax.vec3)
  mesh_normal: array("nmeshnormal", ax.vec3)
  mesh_face: array("nmeshface", ax.vec3i)
  mesh_graph: array("nmeshgraph", int)
  mesh_quat: array("nmesh", ax.quat)
  mesh_polynum: array("nmesh", int)
  mesh_polyadr: array("nmesh", int)
  mesh_polynormal: array("nmeshpoly", ax.vec3)
  ...
  hfield_size: array("nhfield", ax.vec4)
  ...
  mat_texrepeat: array("*", "nmat", ax.vec2)
  mat_rgba: array("*", "nmat", ax.vec4)
  pair_dim: array("npair", int)
  pair_geom1: array("npair", int)
  pair_geom2: array("npair", int)
  pair_solref: array("*", "npair", ax.vec2)
  pair_solreffriction: array("*", "npair", ax.vec2)
  pair_solimp: array("*", "npair", ax.vec5)
  pair_margin: array("*", "npair", float)
  pair_gap: array("*", "npair", float)
  pair_friction: array("*", "npair", ax.vec5)
  ...
  eq_solref: array("*", "neq", ax.vec2)
  eq_solimp: array("*", "neq", ax.vec5)
  eq_data: array("*", "neq", ax.vec11)
  ...
  tendon_solref_lim: array("*", "ntendon", ax.vec2)
  tendon_solimp_lim: array("*", "ntendon", ax.vec5)
  tendon_solref_fri: array("*", "ntendon", ax.vec2)
  tendon_solimp_fri: array("*", "ntendon", ax.vec5)
  tendon_range: array("*", "ntendon", ax.vec2)
  tendon_actfrcrange: array("*", "ntendon", ax.vec2)
  ...
  tendon_lengthspring: array("*", "ntendon", ax.vec2)
  ...
  actuator_trnid: array("nu", ax.vec2i)
  ...
  actuator_dynprm: array("*", "nu", ax.vec10)
  actuator_gainprm: array("*", "nu", ax.vec10)
  actuator_biasprm: array("*", "nu", ax.vec10)
  actuator_actearly: array("nu", bool)
  actuator_ctrlrange: array("*", "nu", ax.vec2)
  actuator_forcerange: array("*", "nu", ax.vec2)
  actuator_actrange: array("*", "nu", ax.vec2)
  actuator_gear: array("*", "nu", ax.spatial_vector)
  actuator_cranklength: array("nu", float)
  actuator_acc0: array("nu", float)
  actuator_lengthrange: array("nu", ax.vec2)
  ...
  plugin_attr: array("nplugin", ax.vec3f)
  ...
  has_sdf_geom: bool
  block_dim: BlockDim
  body_tree: tuple[ax.array(dtype=int), ...]
  mocap_bodyid: array("nmocap", int)
  body_fluid_ellipsoid: array("nbody", bool)
  jnt_limited_slide_hinge_adr: ax.array(dtype=int)
  jnt_limited_ball_adr: ax.array(dtype=int)
  dof_tri_row: ax.array(dtype=int)
  dof_tri_col: ax.array(dtype=int)
  nxn_geom_pair: ax.array(dtype=ax.vec2i)
  nxn_geom_pair_filtered: ax.array(dtype=ax.vec2i)
  nxn_pairid: ax.array(dtype=ax.vec2i)
  nxn_pairid_filtered: ax.array(dtype=ax.vec2i)
  geom_pair_type_count: tuple[int, ...]
  geom_plugin_index: array("ngeom", int)
  eq_connect_adr: ax.array(dtype=int)
  eq_wld_adr: ax.array(dtype=int)
  eq_jnt_adr: ax.array(dtype=int)
  eq_ten_adr: ax.array(dtype=int)
  eq_flex_adr: ax.array(dtype=int)
  tendon_jnt_adr: ax.array(dtype=int)
  tendon_site_pair_adr: ax.array(dtype=int)
  tendon_geom_adr: ax.array(dtype=int)
  tendon_limited_adr: ax.array(dtype=int)
  ten_wrapadr_site: ax.array(dtype=int)
  ten_wrapnum_site: ax.array(dtype=int)
  wrap_jnt_adr: ax.array(dtype=int)
  wrap_site_adr: ax.array(dtype=int)
  wrap_site_pair_adr: ax.array(dtype=int)
  wrap_geom_adr: ax.array(dtype=int)
  wrap_pulley_scale: array("nwrap", float)
  actuator_trntype_body_adr: ax.array(dtype=int)
  sensor_pos_adr: ax.array(dtype=int)
  sensor_limitpos_adr: ax.array(dtype=int)
  sensor_vel_adr: ax.array(dtype=int)
  sensor_limitvel_adr: ax.array(dtype=int)
  sensor_acc_adr: ax.array(dtype=int)
  sensor_rangefinder_adr: ax.array(dtype=int)
  rangefinder_sensor_adr: ax.array(dtype=int)
  sensor_collision_start_adr: ax.array(dtype=int)
  collision_sensor_adr: array("nsensor", int)
  sensor_touch_adr: ax.array(dtype=int)
  sensor_limitfrc_adr: ax.array(dtype=int)
  sensor_e_potential: bool
  sensor_e_kinetic: bool
  sensor_tendonactfrc_adr: ax.array(dtype=int)
  ...
  taxel_sensorid: ax.array(dtype=int)
  qM_tiles: tuple[TileSet, ...]
  qLD_updates: tuple[ax.array(dtype=ax.vec3i), ...]
  qM_fullm_i: ax.array(dtype=int)
  qM_fullm_j: ax.array(dtype=int)
  qM_mulm_i: ax.array(dtype=int)
  qM_mulm_j: ax.array(dtype=int)
  qM_madr_ij: ax.array(dtype=int)
...
class Contact:
  ...
  pos: array("naconmax", ax.vec3)
  frame: array("naconmax", ax.mat33)
  includemargin: array("naconmax", float)
  friction: array("naconmax", ax.vec5)
  solref: array("naconmax", ax.vec2)
  solreffriction: array("naconmax", ax.vec2)
  solimp: array("naconmax", ax.vec5)
  dim: array("naconmax", int)
  geom: array("naconmax", ax.vec2i)
...
class Constraint:
  ...
  quad: array("nworld", "njmax", ax.vec3)
  quad_gauss: array("nworld", ax.vec3)
...
class Data:
  ...
  xfrc_applied: array("nworld", "nbody", ax.spatial_vector)
  eq_active: array("nworld", "neq", bool)
  mocap_pos: array("nworld", "nmocap", ax.vec3)
  mocap_quat: array("nworld", "nmocap", ax.quat)
  qacc: array("nworld", "nv", float)
  act_dot: array("nworld", "na", float)
  sensordata: array("nworld", "nsensordata", float)
  xpos: array("nworld", "nbody", ax.vec3)
  xquat: array("nworld", "nbody", ax.quat)
  xmat: array("nworld", "nbody", ax.mat33)
  xipos: array("nworld", "nbody", ax.vec3)
  ximat: array("nworld", "nbody", ax.mat33)
  xanchor: array("nworld", "njnt", ax.vec3)
  xaxis: array("nworld", "njnt", ax.vec3)
  geom_xpos: array("nworld", "ngeom", ax.vec3)
  geom_xmat: array("nworld", "ngeom", ax.mat33)
  site_xpos: array("nworld", "nsite", ax.vec3)
  site_xmat: array("nworld", "nsite", ax.mat33)
  cam_xpos: array("nworld", "ncam", ax.vec3)
  cam_xmat: array("nworld", "ncam", ax.mat33)
  light_xpos: array("nworld", "nlight", ax.vec3)
  light_xdir: array("nworld", "nlight", ax.vec3)
  subtree_com: array("nworld", "nbody", ax.vec3)
  cdof: array("nworld", "nv", ax.spatial_vector)
  cinert: array("nworld", "nbody", ax.vec10)
  flexvert_xpos: array("nworld", "nflexvert", ax.vec3)
  ...
  wrap_obj: array("nworld", "nwrap", ax.vec2i)
  wrap_xpos: array("nworld", "nwrap", ax.spatial_vector)
  actuator_length: array("nworld", "nu", float)
  actuator_moment: array("nworld", "nu", "nv", float)
  crb: array("nworld", "nbody", ax.vec10)
  qM: ax.array3d(dtype=float)
  qLD: ax.array3d(dtype=float)
  ...
  cvel: array("nworld", "nbody", ax.spatial_vector)
  cdof_dot: array("nworld", "nv", ax.spatial_vector)
  ...
  subtree_linvel: array("nworld", "nbody", ax.vec3)
  subtree_angmom: array("nworld", "nbody", ax.vec3)
  ...
  cacc: array("nworld", "nbody", ax.spatial_vector)
  cfrc_int: array("nworld", "nbody", ax.spatial_vector)
  cfrc_ext: array("nworld", "nbody", ax.spatial_vector)
  ...
  subtree_bodyvel: array("nworld", "nbody", ax.spatial_vector)

  # warp only: collision driver
  collision_pair: array("naconmax", ax.vec2i)
  collision_pairid: array("naconmax", ax.vec2i)
