...
from .mujoco_musa import types as mjmtp
...
def _create_ax_array(data: Any, spec: ax.array, sizes: dict[str, int]) -> Union[ax.array, None]:
  """Creates a warp array and populates it with data.
  ...
  if data is None and shape is None:
    ...
    if np.array(data).size > 0:
      ...
      array = ax.array(shape, dtype=spec.dtype)
...
def _create_array_from_wp(warp_array: wp.array) -> ax.array:
  """Creates a warp musa array from a warp array."""
  array_np = warp_array.numpy()
  ax_type = ax.wp_dtype_to_ax_dtype(warp_array.dtype)
  if array_np.size == 0:
    return ax.array(shape=warp_array.shape, dtype=ax_type)
  else:
    return ax.array(data=array_np, shape=warp_array.shape, dtype=ax_type)
  

def _mjm_type_map(d: object) -> type:
  if isinstance(d, wp.array):
    return ax.array
  elif isinstance(d, types.TileSet):
    return mjmtp.TileSet
  return type(d)

def _create_data_from_wp(dst_type: type, wp_data: object) -> object:
  if isinstance(dst_type, ax.array) or dst_type == ax.array:
    new_data = _create_array_from_wp(wp_data)
  elif dataclasses.is_dataclass(dst_type):
    new_data = _init_mjm_struct(dst_type, wp_data)
  elif isinstance(wp_data, tuple):
    new_data = tuple([_create_data_from_wp(_mjm_type_map(d), d) for d in wp_data])
  else:
    new_data = wp_data
  return new_data

def _init_mjm_struct(musa_struct_type: type, warp_data_src: object) -> object:
  data = musa_struct_type(**{f.name: None for f in dataclasses.fields(musa_struct_type)})
  for f in dataclasses.fields(musa_struct_type):
    wp_data = getattr(warp_data_src, f.name)
    new_data = _create_data_from_wp(f.type, wp_data)
    setattr(data, f.name, new_data)
  return data
...
def put_model(mjm: mujoco.MjModel) -> types.Model:
  ...
  # ################################################################################
  # # Musa specific fields
  # ################################################################################
  m.musa_model = _init_mjm_struct(mjmtp.Model, m)
  # if m.musa_model.opt.cone == mjmtp.ConeType.ELLIPTIC:
  #   m.musa_model.opt.cone = mjmtp.ConeType.PYRAMIDAL
  #   print("WARN: ELLIPTIC cone type not supported yet, using PYRAMIDAL instead.")
  return m
...
def make_data(
  ...
  d.musa_data = _init_mjm_struct(mjmtp.Data, d)
  return d
...
def put_data(
  ...
  contact.geomcollisionid = wp.zeros((naconmax,), dtype=int)  # TODO(team): set values
  ...
  d.ne_connect = wp.full(nworld, int(3 * np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_CONNECT) & mjd.eq_active)), dtype=int)
  d.ne_weld = wp.full(nworld, int(6 * np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_WELD) & mjd.eq_active)), dtype=int)
  d.ne_jnt = wp.full(nworld, int(np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_JOINT) & mjd.eq_active)), dtype=int)
  d.ne_ten = wp.full(nworld, int(np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_TENDON) & mjd.eq_active)), dtype=int)
  d.ne_flex = wp.full(nworld, int(np.sum((mjm.eq_type == mujoco.mjtEq.mjEQ_FLEX) & mjd.eq_active)), dtype=int)
...
def override_model(model: Union[types.Model, mujoco.MjModel], overrides: Union[dict[str, Any], Sequence[str]]):
  ...
  if isinstance(model, types.Model):
    model.musa_model = _init_mjm_struct(mjmtp.Model, model)
