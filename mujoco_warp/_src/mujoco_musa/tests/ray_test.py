...
from mujoco_warp._src.mujoco_musa import ray as ray_musa
...
def normalize(v: tuple) -> ax.vec3:
  assert len(v) == 3
  v = np.array(v)
  return ax.vec3(v / np.linalg.norm(v))

class RayTest(absltest.TestCase):
  def test_ray_nothing(self):
    """Tests that ray returns -1 when nothing is hit."""
    mjm, mjd, m, d = musa_test_data.fixture("ray.xml")

    pnt = ax.array([ax.vec3(12.146, 1.865, 3.895)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([ax.vec3(0.0, 0.0, -1.0)], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
  ...
  def test_ray_plane(self):
    ...
    pnt = ax.array([ax.vec3(2.0, 1.0, 3.0)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.1, 0.2, -1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    ...
    pnt = ax.array([ax.vec3(0.0, 0.0, -0.5)], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    dist_np = dist.numpy()[0, 0]
  ...
  def test_ray_sphere(self):
    ...
    pnt = ax.array([ax.vec3(0.0, 0.0, 1.6)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.1, 0.2, -1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
  ...
  def test_ray_capsule(self):
    ...
    pnt = ax.array([ax.vec3(0.5, 1.0, 1.6)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.0, 0.05, -1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    dist_np = dist.numpy()[0, 0]
    ...
    pnt = ax.array([ax.vec3(-0.5, 1.0, 0.05)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.0, 0.05, 1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    ...
    pnt = ax.array([ax.vec3(0.0, 1.0, 0.75)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((1.0, 0.0, 0.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    dist_np = dist.numpy()[0, 0]
  ...
  def test_ray_cylinder(self):
    ...
    pnt = ax.array([ax.vec3(2.0, 0.0, 0.05)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.0, 0.05, 1.0))], dtype=ax.vec3, shape=(1, 1))
  ...
  def test_ray_box(self):
    ...
    pnt = ax.array([ax.vec3(1.0, 0.0, 1.6)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.0, 0.05, -1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    ...
    pnt = ax.array([ax.vec3(1.0, 0.0, 0.05)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.0, 0.05, 1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
  ...
  def test_ray_mesh(self):
    ...
    pnt = ax.array([ax.vec3(2.0, 2.0, 2.0)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((-1.0, -1.0, -1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    dist_np = dist.numpy()[0, 0]
    ...
    pnt = ax.array([ax.vec3(4.0, 2.0, 2.0)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((2.0, 1.0, 1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    _assert_eq(geomid_np, -1, "geom_id")
  ...
  def test_ray_hfield(self):
    ...
    pnt = ax.array([ax.vec3(0.0, 2.0, 2.0)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([ax.vec3(0.0, 0.0, -1.0)], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec)
  ...
  def test_ray_geomgroup(self):
    ...
    pnt = ax.array([ax.vec3(2.0, 1.0, 3.0)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.1, 0.2, -1.0))], dtype=ax.vec3, shape=(1, 1))
    geomgroup = ax.vec6(1, 0, 0, 0, 0, 0)
    dist, geomid = ray_musa.ray(m, d, pnt, vec, geomgroup=geomgroup)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    dist_np = dist.numpy()[0, 0]
    _assert_eq(geomid_np, 0, "geom_id")
  ...
  def test_ray_flg_static(self):
    ...
    pnt = ax.array([ax.vec3(2.0, 1.0, 3.0)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.1, 0.2, -1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec, flg_static=False)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    dist_np = dist.numpy()[0, 0]
    _assert_eq(geomid_np, -1, "geom_id")
  ...
  def test_ray_bodyexclude(self):
    ...
    pnt = ax.array([ax.vec3(2.0, 1.0, 3.0)], dtype=ax.vec3, shape=(1, 1))
    vec = ax.array([normalize((0.1, 0.2, -1.0))], dtype=ax.vec3, shape=(1, 1))
    dist, geomid = ray_musa.ray(m, d, pnt, vec, bodyexclude=0)
    # ax.synchronize()
    geomid_np = geomid.numpy()[0, 0]
    dist_np = dist.numpy()[0, 0]
    _assert_eq(geomid_np, -1, "geom_id")
  ...
  def test_ray_invisible(self):
    ...
    m.geom_rgba = ax.array2d([[ax.vec4(0.0, 0.0, 0.0, 0.0)] * 8], dtype=ax.vec4)
